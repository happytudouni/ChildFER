Background
============
The "Data and Materials" folder contains the raw MT data for one participant (raw__imageChoice_0941.csv), as well as the correlation values between every pair of emotions from all participants' rating tasks (rating.sav). The "Codes" directory includes files for calculating the maximum deviation of the participant's mouse trajectories under the happy vs. fear condition(MTAnalysis_conditionHA_FE.m).
...
- Additionally, the process for analyzing the rating task in SPSS is not provided here. This analysis does not require coding and can be completed by navigating through the SPSS menu to “Analyze - General Linear Model - Repeated Measures”.
